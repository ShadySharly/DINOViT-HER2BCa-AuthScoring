...
def save_embeddings_on_master(args, epoch, student_output):
    if is_main_process():
        ...
        pt_student_name = f'student_epoch_{epoch}.pt'
        ...
        csv_student_name = f'student_epoch_{epoch}.csv'
        csv_student_path = os.path.join(args.embed_dir, csv_student_name)
        column_names = [f"embedding_{i}" for i in range(student_embeddings.shape[1])]
...
def init_distributed_mode(args):
    ...
    if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:
        print('--> launched with torch.distributed.run')
    ...
    # Setup
    logging.debug(f"Initializing process group for rank {args.rank} with world size {args.world_size}")
    ...
    logging.debug(f"Process {args.rank} is set to use GPU {args.rank}")
