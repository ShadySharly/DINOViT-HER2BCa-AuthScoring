    ...
    * Initialize cudnn: The function initializes the CUDA Deep Neural Network library (cuDNN) using the
    ...
    if utils.is_main_process():
        if args.output_dir == "":
            ...
            elif os.path.exists(args.embed_dir) and args.save_embed:
                try:
                    shutil.rmtree(args.embed_dir)
                    print(
                        f"Directory '{args.embed_dir}' and its contents removed successfully."
                    )
                except OSError as e:
                    print(
                        f"Error: {e}. Failed to remove directory '{args.embed_dir}' and its contents."
                    )
    ...
    logging.debug(f"Process {args.rank} started")
    for epoch in range(start_epoch, args.epochs):
        data_loader.sampler.set_epoch(epoch) # type: ignore

        # ============ training one epoch of DINO ... ============
        student_output, train_stats = train_one_epoch(
            student,
            teacher,
            teacher_without_ddp,
            dino_loss,
        ...
        # ============ storing embeddings ... ============
        if (args.save_embed):
            utils.save_embeddings_on_master(args, epoch, student_output)
...
def train_one_epoch(
    ...
    logging.debug(f"Process {args.rank} at epoch {epoch}")
    ...
    logging.debug(f"Process {args.rank} completed training")
    return (
        student_output,
...
if __name__ == "__main__":
    logging.debug(f"OMP_NUM_THREADS is set to {os.environ.get('OMP_NUM_THREADS')}")
